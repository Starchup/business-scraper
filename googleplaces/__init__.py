...
class GooglePlaces(object):
    ...
    def nearby_search(self, language=lang.ENGLISH, keyword=None, location=None,
        ...
        types    -- An optional list of types, restricting the results to
                    Places (default []).
        pagetoken-- An optional parameter for paging (defaults to None).
        """
        ...
        for place in places_response[u'results']:
            for type in place[u'types']:
                place[u'name'] = place[u'name'].encode('ascii', 'ignore')
...
class GooglePlacesSearchResult(object):
    ...
    def next_page_token(self):
        #if len(self._response.keys()) == 4:
        if "next_page_token" in self._response.keys():
